...
class AsciiImage {
	...
	/**
	 * Liefert die Anzahl der unterschiedlichen Zeichen in diesem AsciiImage.
	 *
	 * @return Anzahl der unterschiedlichen Zeichen.
	 */
	public int getUniqueChars() {
		return getUniqueChars(this.toString());
	}

	/**
	 * Vergleicht das angegebene Bild mit diesem Bild.
	 *
	 * Liefert true wenn Höhe und Breite übereinstimmen und der Pixelwert an jeder Position des angegebenen Bildes mit dem in diesem Bild übereinstimmt.
	 *
	 * @param o Das zu vergleichende AsciiImage.
	 * @return true wenn die beiden Bilder exakt übereinstimmen.
	 */
	 ...
	 *
	 * @return Die Summe der Ascii Codes aller Zeichen im Bild.
	 */
	public int hashCode() {
		int hash = 0;
		for(int x=0; x<getWidth(); x++) {
			for(int y=0; y<getHeight(); y++) {
				hash += (int)getPixel(x,y);
			}
		}
		return hash;
	}
